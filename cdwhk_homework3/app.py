...
def index():
    ...
    if request.method == "POST":
        ...
        total_days_taken = sum(
            [(lr.date_end - lr.date_start).days + 1 for lr in existing_leaves]
        )
        ...
        existing_leave = LeaveRequest.query.filter_by(
            user_id=user_id, date_start=leave_date_start
        ).first()
...
def delete(id):
    ...
    if leave_to_delete.user_id == session.get("user_id"):
        if datetime.utcnow() < leave_to_delete.date_start:
            ...
            return (
                "You cannot delete a leave request that has already started or passed"
            )
    else:
